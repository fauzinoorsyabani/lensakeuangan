  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="overflow-hidden rounded-[1.5rem] border border-[#e3e8e0] bg-white p-4 shadow-[0_10px_30px_rgba(45,69,57,0.05)]">
          ...
          <div className="mt-4 flex items-center justify-between text-xs text-[#77837c]"><span className="truncate pr-4">{review.data.receipt.fileName}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${confidenceTone(extraction.confidence.overall)}`}>Keyakinan {Math.round(extraction.confidence.overall * 100)}%</span></div>
        </article>
        <article className="rounded-[1.5rem] border border-[#e3e8e0] bg-white p-5 shadow-[0_10px_30px_rgba(45,69,57,0.05)] lg:p-6">
          <div className="flex items-start justify-between"><div><p className="font-display text-lg font-bold">Detail transaksi</p><p className="mt-1 text-xs text-[#7a867e]">Ubah setiap field yang tidak sesuai dengan struk.</p></div><span className={`rounded-lg px-2.5 py-1.5 text-[10px] font-bold ${confidenceTone(extraction.confidence.overall)}`}>AI {Math.round(extraction.confidence.overall * 100)}%</span></div>
